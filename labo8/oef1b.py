...
# Maak een visualisatie die de kans op elke combinatie van twee dobbelstenen visualiseert.

# Maak een matrix voor de heatmap
heatmap_matrix = np.zeros((6, 6))
for i in range(6):
    for j in range(6):
        heatmap_matrix[i, j] = np.sum((dice1 == i + 1) & (dice2 == j + 1)) / n_simulations

# Plot de heatmap
plt.figure(figsize=(8, 6))
plt.imshow(heatmap_matrix, cmap='Blues', interpolation='nearest')

# Voeg labels en titels toe
plt.xticks(np.arange(6), np.arange(1, 7))
plt.yticks(np.arange(6), np.arange(1, 7))
plt.xlabel('Dobbelsteen 2')
plt.ylabel('Dobbelsteen 1')
plt.title('Kans op elke combinatie van twee dobbelstenen')

# Voeg waarden toe aan de cellen
for i in range(6):
    for j in range(6):
        plt.text(j, i, f'{heatmap_matrix[i, j]:.3f}', ha='center', va='center', color='black')
